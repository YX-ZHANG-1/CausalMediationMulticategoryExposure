...
##
## Arguments:
##   y             Outcome variable. Must not contain missing values.
##
##   z             Multi-category exposure variable. Must not contain
##                 missing values.
##
##   m             Mediator variable(s). Must not contain missing values.
##                 Can be scalar or vector, and may be binary, categorical,
##                 or continuous.
##
##   x             (Potential) pre-treatment confounders of the exposure,
##                 mediator, and/or outcome. Must not contain missing values.
##
##   j             Active exposure level to be analyzed.
##
##   trim          Trimming threshold for discarding observations with
##                 extreme conditional exposure or mediator probabilities
##                 (or their products). Observations with (products of)
##                 conditional probabilities smaller than this threshold in
##                 any denominator of the potential outcomes are dropped.
##
##   fewsplits     Logical. If TRUE, the same training data are used for
##                 estimating nested nuisance models:
##                   - E(Y | Z = z, M, X)
##                   - E[E(Y | Z = z, M, X) | Z = z', X]
##                 If FALSE, the training data are further split for
##                 sequential estimation of these models.
##
##   normalized    Logical. If TRUE, the inverse-probability weights are
##                 normalized to sum to 1 within each exposure level.
##
##   seed          Random seed value for reproducibility.
##
##   type.measure  Loss function used for cross-validation in cv.glmnet
##                 (e.g., "deviance").
##
...
## Returns:
##   A list containing:
##     - results: A matrix with effect estimates, standard errors,
##                and p-values for total, direct, and indirect effects.
##     - ntrimmed: Number of observations dropped due to trimming.
## =====================================================================
